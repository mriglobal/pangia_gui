...
from annoy_plugin import Annoy
...
def parse_params(ver):
	...
	p.add_argument('-rm', '--readmapper',
			choices=['bwa','minimap2'], type=str, default='minimap2',
			help="Choose a read mapper from bwa or minimap2 [default: 'minimap2']")
	...
	p.add_argument('-st','--sequencerType',
			type=str, required=True,
			choices=['illumina', 'nanopore'],
				help="""R|You can specify either illumina or nanopore sequencer types:\n""")

	p.add_argument('-sm','--scoreMethod',
			type=str, default='standalone',
			choices=['bg', 'standalone', 'combined'],
			help="""R|You can specify one of the following scoring method:\n"""
			     """"bg"         : compare mapping results with the background;\n"""
	...
	p.add_argument( '-M','--markers', action="store_true",
					help="Employ additional marker of interest analysis.")

	p.add_argument( '--annoy', action="store_true",
                    help="Employ additional annoy analysis.")
	...
	p.add_argument('-panml','--PanGIAMLsecondopinion',
                        action='store_true',default=False,
                        help="Activate workflow to use ML models to use to add second opinion(s) for organism detection.  [default: False]")


	default_ML_str = "DT,RF"
	p.add_argument('-panmlm','--PanGIAMLsecondopinionmodels',
                        metavar='<STR>', type=str, default=default_ML_str,
                        help="String that lists the ML models to use to add second opinion(s) for organism detection. By default, only the general purpose Decision Tree (DT) and general purpose Random Forest (RF) are available, though other models can be added. To define which ML models to use add a string that contains the strings for each of the models to use. For example , 'DT', 'DT+', 'onlyDT' will only acitvate DT second opinion. 'DT,RF' will activate both models ( no set delimiter is defined, as long as 'DT' and 'RF' are in string, both will be activated)  [default: %s]"%default_val)
	...
	if args_parsed.sequencerType == 'nanopore':
		if args_parsed.readmapper == "minimap2": #so that if a user sets it to bwa, the user input options don't get reset
			args_parsed.addOptions = "-x map-ont"
			args_parsed.minScore = 0
			args_parsed.minRsnb = 1
			args_parsed.minReads = 3
			args_parsed.minLen = 50 #though these numbers are all dropped low because nanopore reads are fewer and longer. Making the minLen drop not make sense, but the others do.
...
def outputResultsAsReport(res_rollup, o, relAbu, user_taxid, display_fields, score_method, ms, mc, mb, mr, ml, md, mrd, display_all):
	...
	for rank in sorted( major_ranks, key=major_ranks.__getitem__ ):
		...
		for tid in sorted( taxas, key=taxas.__getitem__, reverse=True ):
			...
			score_fields = ""
			if "S_SA" in res_rollup[tid]:
				#this check added to S_SA and S_BG formatting to handle 'NA' score result - Phil
				if type(res_rollup[tid]["S_SA"])==str and not res_rollup[tid]["S_SA"].isnumeric(): res_rollup[tid]["S_SA"] = "none"
			...
			if "S_BG" in res_rollup[tid]:
				if type(res_rollup[tid]["S_BG"])==str and not res_rollup[tid]["S_BG"].isnumeric(): res_rollup[tid]["S_BG"] = "none"
...
def readMapping(reads, dbs, threads, add_options, seed_length, min_score, samfile, logfile, readmapper):
	...
	for db in dbs:
		...
		#if not os.path.isfile():p.error( "%s as  not found. Please check for the existence of this file in the correct location."%() )
...
def getStrainSize(dbs):
	...
	strain={}
	host={}
	#TODO: rewrite to handle general case better i.e. instead of rstrip remove suffix of dbs and then add extension
	if argvs.readmapper == 'bwa':
		default_extension = '.ann'
		default_field_selector=1
	else:
		default_extension = '.headers'
		default_field_selector=0
#TODO: Figure out how to handle BWA case as well
	for db in dbs: #Dave change
		extension = default_extension
		field_selector = default_field_selector
		if not os.path.isfile(db.rstrip(".mmi")+extension):
			extension = '.ann'
			field_selector=1
		#print_message( "possible db:"+"__".join(dbs), argvs.silent, begin_t, logfile, True )
		#print_message( "used db:",db.rstrip(".mmi")+extension, argvs.silent, begin_t, logfile, True )
		with open(db.rstrip(".mmi")+extension) as f:
			for line in f:
				if '|' in line:
					tmp = line.split(' ')[field_selector]
					r = tmp.split('|')
					tid = r[2]
					if 'H' in r[3]:
						host[tid] = True
						...
						if tid in strain:
							...
							strain[tid] = int(r[1])
		#also, this check only covers the scenario where everything is missing, not partially missing
		if len(strain) == 0:
...
##DAVE'S FUNCTIONS##
def make_bam(prefix):
	sam = pysam.AlignmentFile('{0}.header.sam'.format(prefix),'r')
	print('imported sam into make bam function')
	boutfile = pysam.AlignmentFile('{0}.bam'.format(prefix), "wb", template=sam)
	for s in sam:
		boutfile.write(s)
	print('wrote bamfile')
	del sam
	del boutfile #memory management

def savebaminteresctastmpdf(tax):
	spedict = {} #dictionary linking strains to species for tax rollup to species and reducing runtime
	with open(str2spec) as f:
		for line in f:
			(key,val) = line.split(':')
			spedict[key] = val.strip('\n')
	taxdf = pd.DataFrame()
	dbdir = argvs.dbPath
	print("this is the tax: {0}".format(tax))
	if str(tax) in spedict.keys():
		spetax.append(spedict[str(tax)])
		print('This is spetax in the loop: {0}'.format(spetax))
		if ".0" in str(tax): #because the sam file ignores '.0' which is why I previously int()ed it.
			strtax = str(tax).split('.')[0] #for sam file
		else:
			strtax = str(tax)
		sys.stdout.write('Marker analysis beginning on TaxID: {0}'.format(tax))
		msamfile = pd.read_csv(trimsamfile,sep='\t',header=None,usecols=[0,1,2],quoting=csv.QUOTE_NONE)
		subsam = msamfile[msamfile[2].apply(lambda x: x.split('|')[2]) == strtax]
		accessions = list(set(list(subsam[2].apply(lambda x: x.split('|')[0])))) #get the accession number from this string NC_002505.1|2961149|243277|Bc
		del subsam #memory management - medium # could convert above to replace subsam variable with expanded subsam definition
		for acc in accessions:
			sys.stdout.write('Marker analysis beginning on accession Number: {0}'.format(acc))
			####error region####
			try: #the bedfile might not exist - non BSATs, etc. I could include a check for marker taxIDs to prevent that from happening
				bedfile = pybedtools.BedTool('{0}/BSAT_markers/report_mod_markers/{1}.mod.bed'.format(dbdir,acc)) #get the appropriate bedfile for the accession number
				sys.stderr.write("\n"+'{0}/BSAT_markers/report_mod_markers/{1}.mod.bed'.format(dbdir,acc)+"\n")
				#sys.stderr.write("\n"+str(taxdf)+"\n")
				#sys.stderr.write("\n"+str(type(bedfile.intersect))+" "+str(help(bedfile.intersect))+"\n")
				check = open(bedfile.intersect(bam).fn).read() #called check because if it has no length, we assume no intersections are found
				sys.stderr.write("\n"+str(type(check))+" "+str(type(check))+"\n")
				del bedfile # memory management - weak
				if len(check)>0:
					df = pd.DataFrame() #start a dataframe.
					df[0] = check.split('\n') #first column to make all the other columns
					df2 = df[0].str.split('\t',expand=True) #more columns!
					df2.replace("",float('NaN'))
					df2 = df2.dropna()
					df2.drop(df2.tail(1).index,inplace=True) #because the rerplace and dropna don't seem to be working
					taxdf = pd.concat([taxdf,df2])
					noteadd = [] #list to replace the NOTE column in the report.tsv
					#These lines may be redundant because reportfile isn't being written to correctly. Test this.
					for index, value in df2[3].value_counts().iteritems(): #loop over the value counts
						noteadd.append('{0}:{1}'.format(index,value))
					reportfile.loc[reportfile.TAXID == tax, 'MARKERS'] = ';'.join(noteadd) #replace NOTE column
				else: #for all the exampels without markers
					noteadd = 'No marker found'
					reportfile.loc[reportfile.TAXID == tax, 'MARKERS'] = noteadd
			#####error region#####
			except:
				sys.stderr.write('Marker error at acc: {0} and tax: {1}. It is likely not in the strain to species list'.format(acc,tax))
		taxdf.to_csv('{0}/{1}.tmpdf'.format(argvs.tempdir,tax)) #reduce memory problem -MASSIVE
...
if __name__ == '__main__':
	...
	#checks for the existence of certain files
	if type(argvs.input).__name__!='NoneType':
		for fil in argvs.input:
			if not os.path.isfile(fil):print_message(  "%s as input fastq not found. Please check for the existence of this file in the correct location."%(fil), argvs.silent, begin_t, logfile )
	if argvs.loadBg:
		for fil in argvs.loadBg:
			if not os.path.isfile(fil):print_message(  "%s as background to load not found. Please check for the existence of this file in the correct location."%(fil), argvs.silent, begin_t, logfile )
	...
	if argvs.mode == 'class':
		...
		del [res_rollup, out_fp, argvs.relAbu, argvs.mode, argvs.minScore, argvs.minRsnb, argvs.minReads, argvs.minLen, argvs.minDc, argvs.displayAll] #DAVE ADDITION FOR MEMORY MANAGEMENT
		gc.collect() #DAVE ADDITION FOR MEMORY MANAGEMENT


	def addemptyDTdetectcolumn(rl,cname):
		# function to add a column with Null values in order to enforce structuring of the report.tsv for easier parsing and ease of incorporation into GUI
		reporttsvdata=pd.read_csv(rl,sep='\t',keep_default_na=False)
		reporttsvdata[cname] = 'N/A'
		reporttsvdata.to_csv(rl,sep='\t',index=False)
		print_message( "Finished adding default values to %s column and saving results"%(cname), argvs.silent, begin_t, logfile )

	reporttsvloc=outfile
	trimsamfile=samfile.split(".pangia.sam")[0]+".trim.pangia.sam"
	print(trimsamfile)
	os.system("cut -f 1,2,3 {0} > {1}".format(samfile,trimsamfile))



#DAVE'S ADDITIONAL MARKER ANALYSIS
	if argvs.markers:
		sys.stdout.write('Starting marker analysis with these features database directory? {1}\toutdirectory? {2}\tprefix? {3}\tsamfile? {4}\treportfile? {5}\n'.format(argvs.stdout, argvs.dbPath, argvs.outdir, argvs.prefix, samfile, outfile))
		dbdir = argvs.dbPath
		masterheader = ('{0}/master.sam.header'.format(dbdir))
		if not os.path.isfile(masterheader):print_message(  "%s as header for T-MArC analysis not found. Please check for the existence of this file in the correct location."%(masterheader), argvs.silent, begin_t, logfile )

		if os.stat(outfile).st_size == 0:
			sys.stderr.write('The reportfile is empty. Please check on this\n')
		else:
			reportfile = pd.read_csv(outfile,sep='\t') #pull in reportfile for modification
			print(reportfile)
		reportfile['MARKERS'] = '' #set empty column to add marker values
		str2spec = '{0}/BSAT_markers/report_mod_markers/current.species.to.strain.list'.format(dbdir) #set location for dictionary translating strains to species
		print(samfile)
		#trimsamfile=samfile.split(".pangia.sam")[0]+".trim.pangia.sam"
		#print(trimsamfile)
		#os.system("cut -f 1,2,3 {0} > {1}".format(samfile,trimsamfile))
		msamfile = pd.read_csv(trimsamfile,sep='\t',header=None,usecols=[0,1,2],quoting=csv.QUOTE_NONE) #pull in sam file for marker analysis #don't conflict with existing samfile variable
		print(msamfile)
		name = samfile.split('.sam')[0]

		#add headers to sam file
		#get headers to add:
		headers = list(set(list(pd.read_csv(trimsamfile,sep='\t',header=None,usecols=[2],quoting=csv.QUOTE_NONE)[2])))
		with open(masterheader) as f, open('{0}.headers'.format(name), 'w') as o:
			sys.stderr.write('Rebuilding SAM header')
			for line in f:
				if any(key in line for key in headers):
					o.writelines(line)
		with open('{0}.header.sam'.format(name),'w') as wfd:
			for f in ['{0}.headers'.format(name),samfile]:
				with open(f,'r') as fd:
					shutil.copyfileobj(fd, wfd)
		#create bam for intersection -- might be an easier way, but this works.
		make_bam(name)
		sys.stderr.write('created BAMfile')
		#create bam for intersection -- might be an easier way, but this works.
		sam = pysam.AlignmentFile('{0}.header.sam'.format(name),'r')

		#    os.system('ls {0}.bam'.format(name))
		i = 0
		while i < 10:
			i += 1
			try:
				bam = pybedtools.BedTool('{0}.bam'.format(name)) #read in bam as a bedtool -- it does not work if read in as a pysam alignment file
			except OSError as e:
				sys.stderr.write('Marker error is {0}. Retrying: {1}'.format(e,i))
				make_bam(name)


		reportfile = reportfile.astype({'TAXID':float}) #set the type because if you don't, it'll have decimal and that breaks the str.contains later or something
		parentdf=pd.DataFrame()
		spetax = [] #for rollup later
		...
		with open(str2spec) as f:
			for line in f:
				(key,val) = line.split(':')
				spedict[key] = val.strip('\n')
		del f #memory management - weak
		tmptaxa = list(set(list(reportfile[reportfile['LEVEL']=='strain']['TAXID'])))
		taxa = list(set([i for i in tmptaxa if str(i) in spedict.keys()])) #only evaluate taxa we have markers for
		del tmptaxa #memory management - weak
		#print(taxa)
#        pool = Pool(argvs.threads) #make parameter
#        tasks = []
#        for i in range(len(taxa)):
#            tasks.append(pool.apply_async(eval,([taxa[i]])))
#        pool.close()
#        pool.join()
		del msamfile #memory management - medium
		#print('This is tasks: {0}'.format(tasks))
#        output = [p.get() for p in tasks]
		#print('This is output: {0}'.format(output))
		#####this is where the taxid is processed#######
		#sys.stderr.write("\n"+"bed: "+str(bed)+"\n")
		sys.stderr.write("\n"+"taxa list: "+str(taxa)+"\n")
		for i in taxa:
			savebaminteresctastmpdf(i)
		all_files = glob.glob('{0}/*tmpdf'.format(argvs.tempdir))
		print('these are all the files: {0}\n'.format(all_files))
		dfs = []
		for filename in all_files:
			df = pd.read_csv(filename)
			dfs.append(df)
		def isdeeplengthzero(listofdf):
			if len(listofdf) == 0 : return(True)
			for item in listofdf:assert(type(item).__name__=='DataFrame')
			entrycountiszero=True
			for item in listofdf:
				if not item.empty:entrycountiszero=False
			return(entrycountiszero)
		#if dfs: old check
		if not isdeeplengthzero(dfs):
			parentdf = pd.concat(dfs, axis=0, ignore_index=True)
#        for t in output:
#            parentdf = pd.concat([parentdf,t])
			parentdf.replace("",float('NaN'))
			parentdf = parentdf.dropna()
			parentdf = parentdf.reset_index(drop=True)
			parentdf.to_csv('parentdf.tsv',sep='\t',header=None)
			sys.stderr.write('\nDAVE LOOK HERE!\n')
			print(parentdf.columns)
			#to try and get rid of taxid module for portability
			parentdf[5] = parentdf['0'].apply(lambda x: int(spedict[str(float(x.split('|')[2]))]))#moved down to see if it works
			parentdf[6] = parentdf['0'].apply(lambda x: float(x.split('|')[2]))
			#parentdf.to_csv('parentdf.tsv',sep='\t',header=None)
			#print('This is parentdf: {0}'.format(parentdf))
			for tax in list(set(taxa)): #
			#try redoing the strain level markers
				sys.stdout.write('Modifying report file to include strain marker counts')
				subdf = parentdf[parentdf[6] == float(tax)]
				#    print('this is subdf {0}'.format(subdf))
				notes = []
				if not subdf.empty:
					for index, value in subdf['3'].value_counts().iteritems():
						notes.append('{0}:{1}'.format(index,value))
					reportfile.loc[reportfile.TAXID == float(tax), 'MARKERS'] = ';'.join(notes)

			#    print('This is the tax in spetax: {0}'.format(spetax))
        		#    for index, value in parentdf.groupby(0)['3'].value_counts().iteritems():
			spetaxa = list(set(list(parentdf[5])))
			for spetax in spetaxa:
				sys.stdout.write('Modifying report file to include species marker counts')
				spenotes = []
				for index, value in parentdf[parentdf[5] == int(spetax)]['3'].value_counts().iteritems():
					spenotes.append('{0}:{1}'.format(index,value))
				reportfile.loc[reportfile.TAXID == float(spetax), 'MARKERS'] = ';'.join(spenotes)

				#            print(reportfile.NOTE)

			reportfile = reportfile.astype({'TAXID':int}) #reset back to int so gui post processing works
			reportfile.to_csv('{0}'.format(outfile),sep='\t',index=False)
			print_message( "T-MArC marker analysis has completed and results have been saved.", argvs.silent, begin_t, logfile )
		else:
			#sys.stderr.write('No markers found for any organism')
			print_message( "No markers found for any organism.", argvs.silent, begin_t, logfile )
			addemptyDTdetectcolumn(reporttsvloc,"MARKERS")
	else:
		#sys.stdout.write('No additional marker analysis selected')
		print_message( "No additional marker analysis selected.", argvs.silent, begin_t, logfile )
		addemptyDTdetectcolumn(reporttsvloc,"MARKERS")

	## ANNOY ANALYSIS
	if argvs.annoy:
        # Start object oriented class for annoy pipeline
        # Config is located in the annoy.py file, add/change this file for new models
        # For each organism of interest in the config check results file, pull reads from sam, run against annoy
		print_message("Running ANNOY analysis and saving results.", argvs.silent, begin_t, logfile )
		annoy = Annoy()
		annoy.outfile = outfile
		annoy.samfile = trimsamfile
		annoy.dbPath = argvs.dbPath
		annoy.outdir = argvs.outdir
		annoy.tmpdir = argvs.tempdir
		annoy.fastq = argvs.input
		annoy.silent = argvs.silent
		annoy.begin_t = begin_t
		annoy.logfile = logfile

		annoy.Start()

	else:
		print_message("Saving Null ANNOY results.", argvs.silent, begin_t, logfile )
		addemptyDTdetectcolumn(reporttsvloc,"ANNOY_RC")
		addemptyDTdetectcolumn(reporttsvloc,"ANNOY")


	def calcandaddmlcolumn(ml,rl,featurelist,cname):
		# function written to take in a model location (relative to database folder), *.report.tsv location, list of features from report.tsv that the model uses, and the name to give the new column that is added to the report.tsv
		print_message( "%s, %s, %s, %s "%(ml,rl,featurelist,cname), argvs.silent, begin_t, logfile )
		MLresults=[]
		mlmodel=joblib.load(ml)
		reporttsvdata=pd.read_csv(rl,sep='\t',)
		#print_message( "VAR: "+str(mlmodel.fit), argvs.silent, begin_t, logfile )
		#print_message( "VAR: "+str(mlmodel.fit), argvs.silent, begin_t, logfile )
		for  i, row in reporttsvdata.iterrows():
			print_message( str(list(row[featlist])), argvs.silent, begin_t, logfile )
			if list(row[["LEVEL"]])[0]=="species": 
				if "none" in list(row[featlist]):
					print_message( str(list(row[featlist])[-2]), argvs.silent, begin_t, logfile )
					print_message( "NAN?: "+str(pd.isna(list(row[featlist])[-2])), argvs.silent, begin_t, logfile )
					MLresults.append("N/A")
				elif pd.isna(list(row[featlist])[-2]):
					MLresults.append("N/A")
				else:
					print_message( "species: "+str(list(row[featlist])), argvs.silent, begin_t, logfile )
					print_message( str(list(row[featlist])[-2])+" "+str(type(list(row[featlist])[-2])), argvs.silent, begin_t, logfile )
					MLresults.append(mlmodel.predict([list(row[featlist])])[0])
			else: MLresults.append("N/A")
		reporttsvdata[cname]=MLresults
		reporttsvdata.to_csv(reporttsvloc,sep='\t',index=False)
		print_message( "Finished adding results to %s column and saving results"%(cname), argvs.silent, begin_t, logfile )


        ## PanGIA Machine Learning ML ANALYSIS includes DT , RF
	if argvs.PanGIAMLsecondopinion:
		#print_message( argvs.PanGIAMLsecondopinionmodels, argvs.silent, begin_t, logfile )
		print_message( "Begin PAN-ML second opinion analysis with the following model string: "+argvs.PanGIAMLsecondopinionmodels, argvs.silent, begin_t, logfile )
		#print_message( str(argvs), argvs.silent, begin_t, logfile )
		assert(os.path.isdir(argvs.dbPath+"PANMLmodels"))
		#print_message( argvs.dbPath+"PANMLmodels", argvs.silent, begin_t, logfile )#
		if argvs.sequencerType == 'illumina':
			#print_message( argvs.sequencerType, argvs.silent, begin_t, logfile )#
			if argvs.scoreMethod  == "standalone":
				#print_message( argvs.scoreMethod, argvs.silent, begin_t, logfile )#
				featlist=["READ_COUNT",  "READ_COUNT_RNR", "READ_COUNT_RSNB", "LINEAR_COV", "DEPTH_COV", "REL_ABUNDANCE", "TOTAL_BP_MISMATCH", "RPKM", "TOL_RS_READ_CNT", "TOL_NS_READ_CNT","TOL_RS_RNR", "TOL_NS_RNR", "TOTAL_BP_MAPPED", "RS_DEPTH_COV", "SCORE_UNIQ", "SCORE_UNIQ_CUR_LVL"]
				if "DT" in argvs.PanGIAMLsecondopinionmodels:
					#print_message( argvs.PanGIAMLsecondopinionmodels, argvs.silent, begin_t, logfile )#
					modelloc=argvs.dbPath+"PANMLmodels/DT_Illumina_Minimap2_withoutbackground/DT_illumina_minimap2_02192021_m4_withoutbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as DT ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"DT_DETECT")
				else:addemptyDTdetectcolumn(reporttsvloc,"DT_DETECT")
				if "RF" in argvs.PanGIAMLsecondopinionmodels:
					modelloc=argvs.dbPath+"PANMLmodels/RF_Illumina_Minimap2_withoutbackground/RF_illumina_minimap2_02192021_ne50_md8_withoutbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as RF ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"RF_DETECT")
				else:addemptyDTdetectcolumn(reporttsvloc,"RF_DETECT")

			else:
				#print_message( argvs.scoreMethod, argvs.silent, begin_t, logfile )#
				featlist=["READ_COUNT",  "READ_COUNT_RNR", "READ_COUNT_RSNB", "LINEAR_COV", "DEPTH_COV", "REL_ABUNDANCE", "TOTAL_BP_MISMATCH", "RPKM", "TOL_RS_READ_CNT", "TOL_NS_READ_CNT","TOL_RS_RNR", "TOL_NS_RNR", "TOTAL_BP_MAPPED", "RS_DEPTH_COV", "SCORE_UNIQ", "SCORE_BG", "SCORE_UNIQ_CUR_LVL"]
				if "DT" in argvs.PanGIAMLsecondopinionmodels:
					#print_message( argvs.PanGIAMLsecondopinionmodels, argvs.silent, begin_t, logfile )#
					modelloc=argvs.dbPath+"PANMLmodels/DT_Illumina_Minimap2_withbackground/DT_illumina_minimap2_02192021_m4_withbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as DT ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"DT_DETECT")
				else:addemptyDTdetectcolumn(reporttsvloc,"DT_DETECT")
				if "RF" in argvs.PanGIAMLsecondopinionmodels:
					modelloc=argvs.dbPath+"PANMLmodels/RF_Illumina_Minimap2_withbackground/RF_illumina_minimap2_02192021_ne50_md8_withbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as RF ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"RF_DETECT")
				else:addemptyDTdetectcolumn(reporttsvloc,"RF_DETECT")
		elif argvs.sequencerType == 'nanopore':
			#print_message( "Do nanopore, no analysis atm", argvs.silent, begin_t, logfile )
			#print_message( argvs.sequencerType, argvs.silent, begin_t, logfile )#
			if argvs.scoreMethod  == "standalone":
				#print_message( argvs.scoreMethod, argvs.silent, begin_t, logfile )#
				print_message("a", argvs.silent, begin_t, logfile )
				featlist=["READ_COUNT",  "READ_COUNT_RNR", "READ_COUNT_RSNB", "LINEAR_COV", "DEPTH_COV", "REL_ABUNDANCE", "TOTAL_BP_MISMATCH", "RPKM", "TOL_RS_READ_CNT", "TOL_NS_READ_CNT","TOL_RS_RNR", "TOL_NS_RNR", "TOTAL_BP_MAPPED", "RS_DEPTH_COV", "SCORE_UNIQ", "SCORE_UNIQ_CUR_LVL"]
				if "DT" in argvs.PanGIAMLsecondopinionmodels:
					print_message("b", argvs.silent, begin_t, logfile )
					#print_message( argvs.PanGIAMLsecondopinionmodels, argvs.silent, begin_t, logfile )#
					modelloc=argvs.dbPath+"PANMLmodels/DT_Nanopore_Minimap2_withoutbackground/DT_nanopore_minimap2_04072021_md6_withoutbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as DT ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					print_message("c", argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"DT_DETECT")
					print_message("d", argvs.silent, begin_t, logfile )
				else:addemptyDTdetectcolumn(reporttsvloc,"DT_DETECT")
				if "RF" in argvs.PanGIAMLsecondopinionmodels:
					modelloc=argvs.dbPath+"PANMLmodels/RF_Nanopore_Minimap2_withoutbackground/RF_nanopore_04122021_ne50md10_withoutbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as RF ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"RF_DETECT")
				else:addemptyDTdetectcolumn(reporttsvloc,"RF_DETECT")
				...
				if "DT" in argvs.PanGIAMLsecondopinionmodels:
					#print_message( argvs.PanGIAMLsecondopinionmodels, argvs.silent, begin_t, logfile )#
					modelloc=argvs.dbPath+"PANMLmodels/DT_Nanopore_Minimap2_withbackground/DT_nanopore_minimap2_04072021_md6_withbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as DT ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"DT_DETECT")
				else:addemptyDTdetectcolumn(reporttsvloc,"DT_DETECT")
				if "RF" in argvs.PanGIAMLsecondopinionmodels:
					modelloc=argvs.dbPath+"PANMLmodels/RF_Nanopore_Minimap2_withbackground/RF_nanopore_04122021_ne50md10_withbackscore.sav"
					if not os.path.isfile(modelloc):print_message(  "%s as RF ML model not found. Please check for the existence of this file in the correct location."%(modelloc) , argvs.silent, begin_t, logfile )
					calcandaddmlcolumn(modelloc,reporttsvloc,featlist,"RF_DETECT")
				else:addemptyDTdetectcolumn(reporttsvloc,"RF_DETECT")

	else:
		print_message( "no PAN-ML run", argvs.silent, begin_t, logfile )
		addemptyDTdetectcolumn(reporttsvloc,"DT_DETECT")
		addemptyDTdetectcolumn(reporttsvloc,"RF_DETECT")

	os.system("rm {0}".format(trimsamfile))
